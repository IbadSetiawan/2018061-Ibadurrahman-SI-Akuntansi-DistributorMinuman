        ...
        <div class="container-fluid">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <form action="/pengeluaran/pengeluaran_update" method="post">
                        {{csrf_field()}}
                        <input type="hidden" name="id_pengeluaran" value="{{ $pengeluaran->id_pengeluaran }}">
                        ID Pengeluaran :
                        <input type="password" class="form-control form-control-user" required="requeired" name="id_pengeluaran" value="{{ $pengeluaran->id_pengeluaran }}">
                        ID Pegawai :
                        ...
                        ID Pembelian :
                        <select name="id_pembelian" class="form-control form-control-user" required class="from-control">
                            @foreach ($pembelian as $pembelian)
                            <option value="{{ $pembelian->id_pembelian }}">{{ $pembelian->id_pembelian}}</option>
                            @endforeach
                        </select>
                        Merk :
                        <input type="text" name="merk" class="form-control form-control-user" required="requeired" value="{{ $pengeluaran->merk }}">
                        Kas Keluar :
                        <input type="int" name="kas_keluar" class="form-control form-control-user" required="requeired" value="{{ $pengeluaran->kas_keluar }}">
                        Tanggal :
                        <input type="date" name="tanggal" class="form-control form-control-user" required="requeired" value="{{ $pengeluaran->tanggal }}">
